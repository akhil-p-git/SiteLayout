...
class PDFReportGenerator:
    ...
    def _add_cover_page(self):
        ...
        info_data = [
            ...
            ['Total Area:', f"{self.data.project.total_area_m2:,.0f} m² ({self.data.project.total_area_m2/10000:.2f} ha)"],
            ['Report Date:', datetime.fromisoformat(self.data.project.report_date).strftime('%B %d, %Y')],
    ...
    def _add_terrain_analysis(self):
        ...
        # Elevation summary
        self.story.append(Paragraph("Elevation Summary", self.styles['SubsectionHeader']))
        ...
        if self.data.terrain.slope_class_distribution:
            self.story.append(Paragraph("Slope Classification", self.styles['SubsectionHeader']))
    ...
    def _add_asset_schedule(self):
        ...
        # Asset table
        asset_data = [
            ['ID', 'Type', 'Dimensions (m)', 'Area (m²)', 'Cut (m³)', 'Fill (m³)', 'Net (m³)']
        ]
        ...
        col_widths = [0.7*inch, 1.2*inch, 1*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.8*inch]
    ...
    def _add_cost_estimate(self):
        ...
        cost_data.extend([
            [f'Contingency ({costs.contingency_percent:.0f}%)', f"${costs.contingency_amount:,.0f}"],
    ...
    def _add_road_network(self):
        ...
        max_gradient = max(r.gradient_percent for r in self.data.roads) if self.data.roads else 0
        ...
        # Road segment table
        road_data = [
            ['Segment', 'Length (m)', 'Width (m)', 'Gradient (%)', 'Cut (m³)', 'Fill (m³)']
        ]
    ...
    def _add_maps(self):
        ...
        for map_img in self.data.maps:
            ...
            if map_img.scale:
                self.story.append(Paragraph(f"Scale: {map_img.scale}", self.styles['BodyText']))
